...
function rearrangeString(s: string, x: string, y: string): string {
    ...
    return y.repeat(cnt) + res;
};


function rearrangeString_opt(s: string, x: string, y: string): string {
    const rest = [...s].filter(c => c !== y).join("");
    const cnt = s.length - rest.length;
    return y.repeat(cnt) + rest;
}